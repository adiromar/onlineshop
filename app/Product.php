...
{   
    public function scopeNotsuspended($query){
      ...
      return $query->select('products.id', 'productName', 'slug', 'featuredImage', 'rate', 'actualRate','highlights','discountType', 'discountPercent')
    ...
    public static function productSales($count = 8)
    {

        $products = Product::select('products.id', 'productName', 'slug', 'featuredImage', 'rate', 'actualRate')->where('discountPercent', '>', 0)
                    ->join('users', 'users.id', '=', 'products.user_id')
                    ->where('users.suspend', '=', 0)
    ...
    public static function productMostBought($count = 8)
    {

        $products = Product::select('products.id', 'productName', 'slug', 'featuredImage', 'rate', 'actualRate')->orderBy('totalSoldQuantity', 'DESC')
                            ->join('users', 'users.id', '=', 'products.user_id')
                            ->where('users.suspend', '=', 0)
                            ->get()->take($count);
